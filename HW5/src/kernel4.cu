...
void hostFE (float upperX, float upperY, float lowerX, float lowerY, int* img, int resX, int resY, int maxIterations)
{
    ...
    int* d_img;
    size_t img_size = resX * resY * sizeof(int);
    // img memory should be allocated by the caller of this function
    ...
    mandelKernel<<<dimGrid, dimBlock>>>(lowerX, lowerY, stepX, stepY, d_img, resX, maxIterations);
