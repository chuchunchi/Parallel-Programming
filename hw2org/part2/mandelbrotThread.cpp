...
void workerThreadStart(WorkerArgs *const args)
{
    ...
    // Calculate the startRow for this thread
    int startRow = args->threadId * rowsPerThread;

    // If this is the last thread, it should process all remaining rows
    if (args->threadId == args->numThreads - 1)
    {
        rowsPerThread += totalRows % args->numThreads;
    }
    
    // Call mandelbrotSerial with the appropriate arguments
    mandelbrotSerial(
    ...
    printf("Thread %d completed work in %ld milliseconds.\n", args->threadId, duration);
    // printf("Hello world from thread %d\n", args->threadId);
    */

    for (int id=args->threadId; id<args->height; id+=args->numThreads) {
        mandelbrotSerial(
            args->x0, args->y0, args->x1, args->y1,
            args->width, args->height,
            id, 1,
